<template>
	<div class="w-full h-dvh overflow-hidden bg-zinc-950">
		<div class="scroller overflow-hidden rotate-12 fixed pointer-events-none select-none" data-direction="left"
		     data-speed="slow" data-animated='true'>
	...
	<div class="absolute bottom-0 w-full">
		<div class="w-full flex flex-col justify-center items-center">
			<NuxtImg preload format="webp" src="/ota-ku/Logo.webp"
			         class="w-1/2 md:w-56 lg:w-60 xl:w-72 pointer-events-none select-none"
			...
			<div class="mt-1 mb-4 text-center pointer-events-none select-none">
				...
				<h2 class="text-xl lg:text-xl mt-2 leading-none">{{ $t('journey_begins') }}<br>
					{{ $t('unforgettable_adventures') }}</h2>
			</div>
			<div class="flex max-w-48 w-full flex-col gap-2 mt-3 mb-8">
				<NuxtLink type="submit" class="button-primary button" :to="$localeRoute('auth/signIn')">{{ $t('sign_in') }}
				</NuxtLink>
				<span class="text-center text-blue-400 font-semibold font-sans text-sm">{{ $t('or') }}</span>
				<NuxtLink class="link link-primary" :to="$localeRoute('/auth/signUp')">{{ $t('create_account') }}</NuxtLink>
			</div>
