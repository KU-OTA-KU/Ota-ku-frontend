			...
			<form class="space-y-5" @submit.prevent="submitForm">
				<div>
					...
					<div class="mt-1">
						...
						<p v-if="errors.email" class="text-red-500 text-xs mt-1">{{ $t('email_error') }}</p>
					</div>
				</div>
				<div>
					<div class="flex items-center justify-between">
						<label for="password" class="block text-sm font-medium leading-5 text-white">{{ $t('password_label') }}</label>
						<div class="mt-1">
							<NuxtLink class="text-sm text-blue-400" to="/auth/forgotPassword">{{ $t('forgot_password') }}</NuxtLink>
						</div>
					</div>
					<div class="mt-1">
						<input id="password" v-model="password" @input="validateForm" type="password"
						       autocomplete="current-password" required :placeholder="$t('password_placeholder')" class="input input-primary"/>
						<p v-if="errors.password" class="text-red-500 text-xs mt-1">{{ $t('password_error') }}</p>
					</div>
				</div>

				<div>
					<button :disabled="isSubmitDisabled" type="submit" class="button button-primary"
					        :class="{ 'button-disabled': isSubmitDisabled }">{{ $t('submit_button') }}</button>
				</div>
			</form>
